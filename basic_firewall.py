...
from scapy.all import IP, TCP, UDP, Raw, IPv6 # Ensure IPv6 is imported for future use

# --- Configuration: Define your blocking rules here ---
# IPs to block (source OR destination)
BLOCKED_IPS = [
...
def packet_handler(pkt):
    ...
    if TCP in scapy_packet:
        ...
        if dst_port in BLOCKED_PORTS:
            print(f"BLOCKING: Destination TCP Port {dst_port} is on the blacklist.")
            pkt.drop()
            return
        ...
        if src_port in BLOCKED_PORTS:
            ...
            return
        if dst_port in BLOCKED_PORTS:
            print(f"BLOCKING: Destination UDP Port {dst_port} is on the blacklist.")
            pkt.drop()
            return
